    ...
    <div class="col-lg-12">
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                ...
                <a class="btn btn-info" href="{{URL::to('webprofile/categories/create')}}" style="margin: 0cm 0px 0cm 10px;">Tambah</a>
                        ...
                        <tr style="cursor:pointer">
                            ...
                            <td style="text-align:center;">
                                @if($value->is_active == 0)
                                  ...
                                  {{ Form::open(['url'=>route('categories_aktif', ['data'=>Crypt::encrypt($value->id)]), 'method'=>'get', 'id' => 'aktif_'.$value->id, 'style' => 'display: none;']) }}
                                  {{ csrf_field() }}
                                  {{ Form::close() }}
                                @endif
                                @if($value->is_active == 1)
                                  <button class="btn btn-danger btn-xs" id="btn_naktif" data-file="{{$value->id}}"><i class="fa fa-times"></i></button>
                                  {{ Form::open(['url'=>route('categories_naktif', ['data'=>Crypt::encrypt($value->id)]), 'method'=>'get', 'id' => 'non_aktif_'.$value->id, 'style' => 'display: none;']) }}
                                  {{ csrf_field() }}
                                  {{ Form::close() }}
                                @endif

                                <a href="{{ route('categories.edit', ['data'=>Crypt::encrypt($value->id)]) }}" class="btn btn-warning btn-xs"><i class="fa fa-pencil"></i></a>
