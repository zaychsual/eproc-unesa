    ...
    <div class="col-lg-12">
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                ...
                <a class="btn btn-info" href="{{URL::to('webprofile/slider/create')}}" style="margin: 0cm 0px 0cm 10px;">Tambah</a>
                ...
                <table class="table datatable table-hover" id="post">
                    <thead>
                        <tr>
                            ...
                            <th width="38%" style="text-align: center;">Slider</th>
                    ...
                    @foreach($data as $value)
                        <tr style="cursor:pointer">
                            ...
                            <td style="vertical-align: middle;">
                              <img src="https://statik.unesa.ac.id/profileunesa_konten_statik/uploads/{!! Session::get('ss_setting')['statik_konten'] !!}/slider/{!!$value->images!!}" alt="" height="100" width="auto">
                            </td>
                            ...
                            <td style="text-align:center; vertical-align: middle;">
                              @if($value->is_active == 0)
                                ...
                                {{ Form::open(['url'=>route('slider_aktif', ['data'=>Crypt::encrypt($value->id)]), 'method'=>'get', 'id' => 'aktif_'.$value->id, 'style' => 'display: none;']) }}
                                {{ csrf_field() }}
                                {{ Form::close() }}
                              @endif
                              @if($value->is_active == 1)
                                <button class="btn btn-danger btn-xs" id="btn_naktif" data-file="{{$value->id}}"><i class="fa fa-times"></i></button>
                                {{ Form::open(['url'=>route('slider_naktif', ['data'=>Crypt::encrypt($value->id)]), 'method'=>'get', 'id' => 'non_aktif_'.$value->id, 'style' => 'display: none;']) }}
                                {{ csrf_field() }}
                                {{ Form::close() }}
                              @endif

                              <a href="{{ route('slider.edit', ['data'=>Crypt::encrypt($value->id)]) }}" class="btn btn-warning btn-xs"><i class="fa fa-pencil"></i></a>

                              <button class="btn btn-danger btn-xs" id="btn_delete" data-file="{{$value->id}}"><i class="fa fa-trash-o"></i></button>
                              {{ Form::open(['url'=>route('slider.destroy', ['data'=>Crypt::encrypt($value->id)]), 'method'=>'delete', 'id' => $value->id, 'style' => 'display: none;']) }}
                              {{ csrf_field() }}
                              {{ Form::close() }}
                            </td>
                           <?php $no++;?>
