    ...
    <div class="col-md-12">
      <div class="content-frame-top">
        <div class="page-title">
            <h3><span class="fa fa-question-circle"></span> Pajak </h3>
        </div>                        
      </div>
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                ...
                <a class="btn btn-info" onclick="window.parent.openPopup('{{URL::to('webprofile/pajaks/create')}}')" style="margin: 0cm 0px 0cm 10px;">Tambah</a>
                ...
                <table class="table datatable table-hover" id="berita">
                    ...
                    @foreach($data as $value)
                        <tr style="cursor:pointer">
                            ...
                            <td>{!! $value->jenis !!}</td>
                            <td>{!! $value->tanggal_bukti !!}</td>
                            <td>{!! $value->nomor_bukti !!}</td>
                            <td style="text-align:center;">
                                

                                <a onclick="window.parent.openPopup('{{ route('pajaks.edit', ['data'=>Crypt::encrypt($value->id)]) }}')" class="btn btn-warning btn-xs"><i class="fa fa-pencil"></i></a>

                                <button class="btn btn-danger btn-xs" id="btn_delete" data-file="{{$value->id}}"><i class="fa fa-trash-o"></i></button>
                                {{ Form::open(['url'=>route('pajaks.destroy', ['data'=>Crypt::encrypt($value->id)]), 'method'=>'delete', 'id' => $value->id, 'style' => 'display: none;']) }}
                                {{ csrf_field() }}
                                {{ Form::close() }}
                              </td>
                          <?php $no++;?>
