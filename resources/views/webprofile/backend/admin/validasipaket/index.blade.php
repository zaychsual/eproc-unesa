    ...
    <div class="col-md-12">
        <div class="content-frame-top">
          <div class="page-title">
              <h3><span class="fa fa-comments"></span> Pemilik </h3>
          </div>                        
        </div>
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                ...
                <a class="btn btn-info" onclick="window.parent.openPopup('{{URL::to('webprofile/validasi-paket/create')}}')" style="margin: 0cm 0px 0cm 10px;">Tambah</a>
                ...
                <table class="table datatable table-hover" id="berita">
                    ...
                    @foreach($data as $value)
                        <tr style="cursor:pointer">
                            ...
                            <td>{{ $value->getPengadaan['jenispengadaan'] ?? '' }}</td>
                            <td>{!! $value->nilai_hps !!}</td>
                            <td style="text-align:center;">
                                

                                <a onclick="window.parent.openPopup('{{ route('pemiliks.edit', ['data'=>Crypt::encrypt($value->id)]) }}')" class="btn btn-warning btn-xs"><i class="fa fa-pencil"></i></a>

                                <button class="btn btn-danger btn-xs" id="btn_delete" data-file="{{$value->id}}"><i class="fa fa-trash-o"></i></button>
                                {{ Form::open(['url'=>route('pemiliks.destroy', ['data'=>Crypt::encrypt($value->id)]), 'method'=>'delete', 'id' => $value->id, 'style' => 'display: none;']) }}
                                {{ csrf_field() }}
                                {{ Form::close() }}
                              </td>
                            <?php //$no++;?>
