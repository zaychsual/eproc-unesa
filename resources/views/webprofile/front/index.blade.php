...
<style>
    ...
    <section class="grey-wrapper jt-shadow">
        <div class="container">
            <div id="content" class="col-lg-6 col-sm-12 col-xs-12">
                <div class="panel panel-primary">
                    <div class="panel-heading">Non-Tender</div>
                    <div class="panel-body">
                        <div class="panel-group" id="accordion">
                            @php
                                $_kategori = \DB::table('e_jenispengadaan')->get();
                            @endphp
                            @foreach($_kategori as $key => $value)              
                            @php
                                $_paket = \App\Models\Procurement\Pakets::where('jenispengadaan_id', $value->id)
                                        ->leftJoin('e_paket_category','e_paket_category.id','=','e_paket.category_id')
                                        ->where('e_paket_category.code','<>','TE')
                            ...
                            <div class="panel panel-primary" id="">
                                <div class="panel-heading">
                                    <h4 class="panel-title">
                                        <a data-toggle="collapse" data-target="#collapseOne-{{ $key }}" 
                                        href="#collapseOne">
                                            <p style="color:white;">{{ $value->jenispengadaan }} <span class="badge badge-danger">
                                            {{ count($_paket) }}
                                            </span></p>
                                        </a>
                                    </h4>
                                </div>
                                <div id="collapseOne-{{ $key }}" class="panel-collapse collapse @if(count($_paket) > 0) in @endif">
                                    <div class="panel-body">
                                    @if(count($_paket) > 0)
                                        <table class="table table-condesed table-bordered">
                                            ...
                                            <tbody>
                                                @foreach($_paket as $key => $value)
                                                    <tr>
                                                        <td>{{ $key + 1 }}</td>
                                                        <td>{{ $value->nama }}</td>
                                                        <td>{{ $value->nilai_hps }}</td>
                                                        {{-- <td>{{ InseoHelper::tglbulanindo2($value->setting_unduh_tutup) }}</td> --}}
                                                    </tr>
                                                @endforeach
                                            </tbody>
                                        </table>
                    ...
                    <div class="panel-body">
                        ...
                        @php
                            $_paket = \App\Models\Procurement\Pakets::where('jenispengadaan_id', $value->id)
                                    ...
                                    ->get();
                        @endphp                  
                        <div class="panel panel-danger" id="">
                            <div class="panel-heading">
                                <h4 class="panel-title">
                                    <a data-toggle="collapse" data-target="#collapseOnes-{{ $key }}" 
                                    href="#collapseOne">
                                        <p style="color:white;">{{ $value->jenispengadaan }} <span class="badges badge-danger">
                                        {{ count($_paket) }}
                                        </span></p>
                                    </a>
                                </h4>
                            </div>
                            <div id="collapseOnes-{{ $key }}" class="panel-collapse collapse @if(count($_paket) > 0) in @endif">
                                <div class="panel-body">
                                    @if(count($_paket) > 0)
                                    <table class="table">
                                        <thead>
    ...
    <section class="grey-wrapper jt-shadow">
        <div class="container">
            ...
            <div id="sidebar" class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                ...
                <a href="{!! url('informasi') !!}" class="btn btn-primary">Lihat Informasi Selengkapnya</a>
                </div><!-- end widget -->
    ...
    <section class="make-bg-full">
        <div class="calloutbox-full-mini nocontainer">
          <div class="long-twitter">
            <p class="lead"><i class="fa fa-twitter"></i> e-Procurement <b>Universitas Negeri Surabaya</b></p>
            </div>
        </div><!-- end calloutbox -->
    </section><!-- make bg -->
@endsection