        ...
        {!! csrf_field() !!}
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                            ...
                            <table class="table table-responsive">
                                ...
                                <tbody id="hps">
                                    <tr>
                                        ...
                                        <td>
                                            <input type="text" name="qty[]" id="qty_0">
                                        </td>
                                        <td>
                                            <input type="text" name="harga[]" id="harga_0">
                                        </td>
                                        <td>
                                            <input type="text" name="pajak[]" id="pajak_0">
                                        </td>
                                        <td>
                    ...
                    <div class="form-group">
                        <label class="col-md-3 col-xs-12 control-label">Rekanan</label>
                        <div class="col-md-6 col-xs-12">
                            {{ Form::select('rekanan_id', $data['rekanan_id'], old('rekanan_id'), ['class' => 'form-control rekanan_id', 'data-live-search' => 'true', 'style' => 'width: 100%;', 'id' => 'rekanan_id', 'placeholder' => '- Pilih Data -', 'required']) }}
                            @if ($errors->has('rekanan_id'))
                                <span class="help-block">{{$errors->first('rekanan_id')}}</span>
                            @endif
                        </div>
                    </div>

                    <div class="form-group">
                        <label class="col-md-3 col-xs-12 control-label" id="detail-rekan" style="display:none">Detail Rekanan</label>
                        <div class="col-md-6 col-xs-12">
                            <table class="table rekanan" id="rekanan" style="display:none">
                                <thead>
                                    <tr>
                                        <th>Kode*</th>
                                        <th>Nama*</th>
                                ...
                                <tbody id="tbDataLokasi">
                                    <tr>
                                        <td>
                                            {{ Form::text('kode_rekanan', old('kode_rekanan'), array('class' => 'form-control', 'id'=>'kode_rekanan', 'required')) }}
                                            @if ($errors->has('provinsi_id'))
                                            <span class="help-block">{{$errors->first('kode_rekanan')}}</span>
                                            @endif
                                        </td>
                                        <td>                                           
                                            {{ Form::text('nama_rekanan', old('nama_rekanan'), array('class' => 'form-control', 'id'=>'nama_rekanan', 'required')) }}
                                            @if ($errors->has('kota_id'))
                                            <span class="help-block">{{$errors->first('nama_rekanan')}}</span>
                                            @endif
                                        </td>
                                        <td>                                           
                                            {{ Form::text('email_rekanan', old('email_rekanan'), array('class' => 'form-control', 'id'=>'email_rekanan', 'required')) }}
                                            @if ($errors->has('kota_id'))
                                            <span class="help-block">{{$errors->first('email_rekanan')}}</span>
                                            @endif
                                        </td>
                                        <td>                                           
                                            {{ Form::text('npwp_rekanan', old('npwp_rekanan'), array('class' => 'form-control', 'id'=>'npwp_rekanan', 'required')) }}
                                            @if ($errors->has('kota_id'))
                                            <span class="help-block">{{$errors->first('npwp_rekanan')}}</span>
                                            @endif
                                        </td>
                                        <td>                                           
                                            {{ Form::text('no_telepon_rekanan', old('no_telepon_rekanan'), array('class' => 'form-control', 'id'=>'no_telepon_rekanan', 'required')) }}
                                            @if ($errors->has('kota_id'))
                                            <span class="help-block">{{$errors->first('no_telepon_rekanan')}}</span>
                                            @endif
                                        </td>
                                        <td>                                           
                                            {{ Form::text('no_hp_rekanan', old('no_hp_rekanan'), array('class' => 'form-control', 'id'=>'no_hp_rekanan', 'required')) }}
                                            @if ($errors->has('kota_id'))
                                            <span class="help-block">{{$errors->first('no_hp_rekanan')}}</span>
                                            @endif
                                        </td>
                                        <td>                                           
                                            {{ Form::textarea('alamat_rekanan', old('alamat_rekanan'), array('class' => 'form-control', 'rows' => 2, 'cols' => 40, 'id'=>'alamat_rekanan', 'required')) }}
                                                @if ($errors->has('alamat_rekanan'))
                                                <span class="help-block">{{$errors->first('alamat_rekanan')}}</span>
                                                @endif
                                        </td>
                                    </tr>
                                </tbody>
                    ...
                    <div class="form-group">
                        <label class="col-md-3 col-xs-12 control-label">File Transaksi</label>
                        <div class="col-md-6 col-xs-12">  
                            {{ Form::file('link_file_dok_pengadaan[]', array('class'=>'fileinput btn-primary', 'id'=>'link_file_dok_pengadaan', 'data-filename-placement'=>'inside', 'title'=>'Browse file', 'required', 'multiple')) }}                                   
    ...
    $("#rekanan_id").change(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $("#rekanan").show();
                $("#detail-rekan").show();
                $("#kode_rekanan").val(response.kode).prop('disabled', true);
                $("#nama_rekanan").val(response.nama).prop('disabled', true);
                $("#email_rekanan").val(response.email).prop('disabled', true);
                $("#npwp_rekanan").val(response.npwp).prop('disabled', true);
                $("#no_telepon_rekanan").val(response.telepon).prop('disabled', true);
                $("#no_hp_rekanan").val(response.hp).prop('disabled', true);
                $("textarea#alamat_rekanan").html(response.alamat).prop('disabled', true);
            }
        })
    })
