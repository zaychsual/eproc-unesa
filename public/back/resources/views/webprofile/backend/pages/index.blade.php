    ...
    <div class="col-lg-12">
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                ...
                <a class="btn btn-info" href="{{URL::to('webprofile/pages/create')}}" style="margin: 0cm 0px 0cm 10px;">Tambah</a>
                ...
                <table class="table datatable table-hover" id="post">
                    <thead>
                        <tr>
                            ...
                            <th width="53%" style="text-align: center;">Judul</th>
                    ...
                    @foreach($data as $value)
                        <tr style="cursor:pointer">
                            ...
                            <td style="vertical-align: middle; font-size: 12pt;">
                              @if($value->thumbnail)
                              <img src="https://statik.unesa.ac.id/profileunesa_konten_statik/uploads/{!! Session::get('ss_setting')['statik_konten'] !!}/pages/{!!$value->thumbnail!!}" alt="" height="100" width="auto"><br>
                              @endif
                              <b>{!! $value->title !!} @if($value->posts_status == 0) - <span style="color: red;">Konsep</span> @endif</b>
                            </td>
                            <td style="vertical-align: middle;">
                              Petonton : {!! $value->viewer !!}<br>
                              Komentar : {!! $value->comment_count !!}
                            </td>
                            <td style="text-align:center; vertical-align: middle;">
                                <a href="{{ route('pages.edit', ['data'=>Crypt::encrypt($value->id)]) }}" class="btn btn-warning btn-xs"><i class="fa fa-pencil"></i></a>

                                <button class="btn btn-danger btn-xs" id="btn_delete" data-file="{{$value->id}}"><i class="fa fa-trash-o"></i></button>
                                {{ Form::open(['url'=>route('pages.destroy', ['data'=>Crypt::encrypt($value->id)]), 'method'=>'delete', 'id' => $value->id, 'style' => 'display: none;']) }}
                                {{ csrf_field() }}
                                {{ Form::close() }}
                            </td>
                           <?php $no++;?>
